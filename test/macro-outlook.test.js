...
test('official US schedule parsers normalize CPI, PCE, payroll and FOMC dates', () => {
  assert.deepEqual(parseBlsReleaseSchedule(BLS_CPI_SCHEDULE_HTML, 'cpi').map((item) => ({
    ...
    { date: '2026-09-11', eventType: 'cpi', referencePeriod: '2026年8月', timeLabel: '20:30' },
    { date: '2026-10-14', eventType: 'cpi', referencePeriod: '2026年9月', timeLabel: '20:30' },
  ]);
  ...
  assert.deepEqual(parseFomcMeetingSchedule(FOMC_CALENDAR_HTML).map((item) => ({
    ...
    { officialStartDate: '2026-09-15', officialDate: '2026-09-16', date: '2026-09-17', timeLabel: '02:00', projections: true },
    { officialStartDate: '2026-10-27', officialDate: '2026-10-28', date: '2026-10-29', timeLabel: '02:00', projections: false },
    { officialStartDate: '2027-01-26', officialDate: '2027-01-27', date: '2027-01-28', timeLabel: '03:00', projections: false },
  ]);
  const januaryFomc = US_ECONOMIC_CALENDAR_SNAPSHOT.find((item) => item.eventType === 'fomc' && item.officialDate === '2027-01-27');
  assert.equal(januaryFomc.timeLabel, '03:00');
});
...
test('macro outlook query returns all independent chart payloads', async () => {
  const fredData = {
    ...
    SAHMCURRENT: 'observation_date,SAHMCURRENT\n2026-06-01,0.23\n2026-07-01,0.27\n',
  ...
  assert.equal(result.charts.find((chart) => chart.id === 'sahmRule').items.at(-1).value, 0.27);
...
test('one failed source does not prevent the remaining charts from loading', async () => {
  const fetchImpl = async (url) => {
    ...
    const id = ['DGS10', 'DGS30', 'DFF', 'CPIAUCSL', 'CBBTCUSD', 'DEXJPUS', 'DEXUSEU', 'DEXUSUK', 'DEXCAUS', 'DEXSDUS', 'DEXSZUS', 'DCOILBRENTEU', 'DCOILWTICO', 'PCOPPUSDM', 'DHHNGSP', 'NASDAQ100', 'SP500', 'VIXCLS', 'T10Y2Y', 'BAMLH0A0HYM2', 'ICSA', 'UNRATE', 'SAHMCURRENT', 'NFCI']
  ...
  assert.equal(result.charts.find((chart) => chart.id === 'sahmRule').error, null);
