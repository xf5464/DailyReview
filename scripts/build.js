...
function applyChartPresentationOverrides() {
  ...
  const scrollLockHelpers = [
    ...
    '  }',
    '',
    '  function dialogScrollContainer(target, dialog) {',
    ...
    '  }',
    '',
    '  function guardDialogTouchStart(event) {',
    "    if (!document.querySelector('dialog[open]') || !event.touches || !event.touches.length) return;",
    '    dialogTouchStartY = event.touches[0].clientY;',
    '  }',
    '',
    '  function guardDialogTouchMove(event) {',
    "    if (!document.querySelector('dialog[open]')) return;",
    "    var target = event.target && event.target.nodeType === 1 ? event.target : event.target && event.target.parentElement;",
    "    var dialog = target && target.closest ? target.closest('dialog[open]') : null;",
    '    if (!dialog) {',
    ...
    '    var deltaY = currentY - dialogTouchStartY;',
    '    var scroller = dialogScrollContainer(target, dialog);',
    ...
    "    document.addEventListener('touchstart', guardDialogTouchStart, { passive: true });",
    "    document.addEventListener('touchmove', guardDialogTouchMove, { passive: false });",
...
function addAssetVersions() {
  ...
  const shellVersion = contentHash([
    'index.html', 'styles.css', 'app.js', 'vendor/fflate.min.js', 'service-worker.js', 'manifest.webmanifest',
