...
function parseFredMdNewOrders(text) {
  ...
  if (headerIndex < 0) throw new Error(`FRED-MD missing ${SERIES_ID}`);
  const header = rows[headerIndex];
  const valueIndex = header.indexOf(SERIES_ID);
  const dateIndex = 0;
  const items = rows.slice(headerIndex + 1).map((row) => {
    const date = normalizeFredMdDate(row[dateIndex]);
    const value = Number(String(row[valueIndex] ?? '').trim());
    return date && Number.isFinite(value) && value >= 0 && value <= 100 ? { date, value } : null;
  }).filter(Boolean);
  if (!items.length) throw new Error('FRED-MD returned no usable ISM new orders data');
  return [...new Map(items.map((item) => [item.date.slice(0, 7) + '-01', {
    date: item.date.slice(0, 7) + '-01',
    value: item.value,
...
async function supplement(options = {}) {
  ...
  const longHistory = parseFredMdNewOrders(text);
  const merged = mergeDatedItems(longHistory, existingItems);
  const cutoff = historyCutoff(merged);
  chart.items = merged.filter((item) => item.date >= cutoff);
  chart.sourceName = 'ISM 官方月报 / FRED-MD 长期历史 / DBnomics 近期历史';
  ...
  process.stdout.write(`Extended ISM new orders history to ${chart.historyStart || 'unknown'} using FRED-MD (${chart.items.length} monthly points).\n`);
  return chart;
}
